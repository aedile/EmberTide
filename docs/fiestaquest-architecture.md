...
**v2.23 amendment (Phase 23 Audio + UX Foundation — feat/phase-23-ux-audio-foundation):** Vendored components: `codec_board` component added (factory port for I2S + ES8311 codec init on ESP32-S3 target); `esp_codec_dev` compiled from source (NOT managed component — required to apply PSRAM alignment fix; managed-components version silently omits the fix). Real micromod player: `game/lib/micromod_real.c` and `game/lib/micromod_real.h` — vendored public-domain micromod decoder by Martin Cameron; global state ADR exception granted: micromod decoder owns one static `Micromod` struct (unavoidable per decoder design); documented in ADR and guarded by `micromod_init()` / `micromod_deinit()` lifecycle contract. hal_audio.c rewritten: factory codec init sequence (codec_board + esp_codec_dev pipeline); ring buffer enlarged to AUDIO_RING_BUF_SAMPLES=44100 samples (44100 Hz stereo fill budget); `hal_audio_get_ring_count()` accessor added to hal_audio.h for prefill-status queries. Build configuration: `CONFIG_BSP_AUDIO` defined via CMake `target_compile_definitions` on the `fq_hal` target (NOT via sdkconfig — avoids Kconfig dependency chain). IDF version requirement raised to ESP-IDF 5.5.1: GDMA bugs #15533 and #8dfcafa (DMA descriptor alignment on PSRAM) fixed in this release; earlier 5.x releases exhibit silent audio corruption on PSRAM-backed ring buffers. sdkconfig.defaults additions: `CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL=16384` (keep small allocs in internal SRAM), `CONFIG_SPIRAM_MALLOC_RESERVE_INTERNAL=32768` (reserve 32 KB internal SRAM for ISR/DMA paths). E-paper driver: lab-tuned partial refresh sequence restored — 0xCF activation command, 0x37 register write, border waveform byte 0x80; sequence was incorrectly dropped in Phase 22 polish pass; restoring it eliminates ghost artifacts on partial refresh. Button label correction: PWR button (GPIO0 / BTN_A) = Move action; SUN button (GPIO18 / BTN_B) = OK/Confirm action; previous mapping was inverted — corrected in hal_pins.h constants and all screen hint strings. Initialization ordering: audio init MUST precede e-paper init in app_main.c HAL init block — GDMA channels are allocated in init order; audio DMA requires channel 0 (highest priority); e-paper SPI DMA takes the next available channel. Reversing order causes audio underrun on first e-paper flush. I2S pin correction: WS=GPIO38, DIN=GPIO16 per board_cfg.h (previous stubs used incorrect GPIO numbers from datasheet errata). Prefill policy: hal_audio ring buffer is prefilled with silence before any blocking e-paper flush call to prevent audible dropout during the ~200 ms e-paper refresh window. SFX mixing: `s_sfx_pending` static buffer holds one pending SFX frame; on each audio callback tick the SFX frame is mixed into the music stream at 70% ducking (music amplitude scaled to 0.7x while SFX is active); ducking uses integer multiply-shift: `sample = (sample * 179) >> 8` (179/256 ≈ 0.699).

**v2.20 amendment (Phase 21 I2S Audio Engine + sfxr-c SFX — feat/phase-21-audio-sfx):** hal_audio.h extended: AUDIO_RING_BUF_SAMPLES=4096u, AUDIO_SAMPLE_RATE_HZ=22050u, AUDIO_MAX_TONE_MS=2000u. New API: hal_audio_write_samples(const int16_t *buf, size_t count) for raw PCM output; duration clamp on hal_audio_play(); ERR_NULL=3, ERR_OVERFLOW=4 appended to hal_audio_err_t (_Static_assert ABI pinning). Target stub hal_audio.c: SPSC ring buffer (static int16_t s_ring_buf[4096], internal SRAM), NULL/init guards, I2S/ES8311 wiring stubs behind CONFIG_BSP_AUDIO. mock_hal_audio.c updated: sample capture buffer MOCK_AUDIO_CAPTURE_MAX=8192, mock_audio_get_samples_written/overflow_flag/capture_buf accessors, ring fill tracking, duration clamped before capture. game/lib/sfxr.h + sfxr.c: vendored minimal sfxr-c PCM generator; sfxr_params_t (wave, ADSR envelope floats, base_freq_hz, freq_slide, total_ms); sfxr_generate() — pure function, no malloc, float intermediates, Bhaskara-I sine approximation, LCG noise (not combat PRNG), SFXR_MAX_DURATION_MS=500u. Architecture boundary: sfxr.h MUST NOT include hal_*.h (enforced by CMake include isolation). game/include/sfx.h: fq_sfx_id_t enum (SFX_BTN_PRESS=0..SFX_REBIRTH=9, SFX_COUNT=10); fq_sfx_play(id, buf, buf_len, samples_out) returns game_err_t. game/src/sfx.c: static const sfxr_params_t k_sfx_presets[10] — 10 distinct retro presets tuned for 22050 Hz. game/CMakeLists.txt: src/sfx.c and lib/sfxr.c registered; lib/ added to INCLUDE_DIRS. app_fsm.h: sfx_enabled (uint8_t) + _p21_pad[3] appended to fq_app_ctx_t; struct size 424 bytes on 64-bit host (Phase-21 fields absorbed existing tail padding), 412 bytes on 32-bit target; _Static_assert updated. app_fsm.c: fq_app_init() sets sfx_enabled=1u by default. app_main.c: includes sfx.h (game) and hal_audio.h (HAL); hal_audio_init() called in HAL init block; do_sfx() helper generates PCM and pushes to ring; SFX wired: BTN_PRESS, BTN_BACK on button_callback; MENU_NAVIGATE on home menu cycle; ITEM_EQUIP on inventory BTN_A; COMBAT_HIT/CRIT on COMBAT_ROUND_COMPLETE; LEVEL_UP on player level increase; DEATH on BATTLE->BATTLE_RESULT when is_dead; REBIRTH on REBIRTH->HOME. test/host/CMakeLists.txt: GAME_SOURCES glob extended to include game/lib/*.c; add_p21_test() helper links mock_hal_audio.c + game sources + HAL include. test/visual/CMakeLists.txt: game/lib glob and include path added. All 94 host tests run; 92 pass (2 pre-existing failures in test_p19_home_menu_feature and test_p19_interactive_bounds unchanged). 14/14 visual golden baselines pass. idf.py build: 371 KB binary, 76% free.
...
**v2.21 amendment (Phase 22 micromod Music + Mixing — feat/phase-22-mod-music):** (1) Vendored library — `game/lib/micromod.h` + `micromod.c`: integer-only ProTracker MOD renderer. All pitch interpolation uses fixed-point 16.16 math (`uint32_t position`, `uint32_t step` fields in channel state). No floating-point operations anywhere in the renderer. No malloc — all state held in `micromod_ctx_t` (caller-owned, stack or static). `MAX_MOD_FILE_SIZE = 524288u` (512 KB). Error enum: `MICROMOD_ERR_OK=0`, `ERR_NULL=1`, `ERR_FORMAT=2`, `ERR_TOO_LARGE=3`. Architecture boundary enforced by CMake include isolation: `micromod.h` MUST NOT include any `hal_*.h` header. (2) Game module — `game/include/music.h` + `game/src/music.c`: `fq_music_ctx_t` wraps `micromod_ctx_t` plus an `initialised` flag. Struct is caller-owned — no mutable globals in `game/`. `_Static_assert` pins size to `sizeof(micromod_ctx_t) + 8u`. API: `fq_music_init(ctx, mod_data, mod_len)` — calls `micromod_init`, sets `initialised=1` on success; `fq_music_play(ctx)` — validates initialised, sets playing flag; `fq_music_stop(ctx)` — clears playing flag; `fq_music_render(ctx, out_buf, sample_count, volume)` — calls `micromod_render`, applies volume scaling `output[i] = (raw_sample * volume) / 256` (int32 intermediate, no float); `fq_music_is_playing(ctx)` — accessor. NULL guards on all entry points return `FQ_MUSIC_ERR_NULL`. (3) Game module — `game/include/music_table.h` + `game/src/music_table.c`: static const track category table. `fq_music_cat_t` enum: `FQ_MUSIC_CAT_CHILL=0` (home/idle), `FQ_MUSIC_CAT_INTENSE=1` (combat), `FQ_MUSIC_CAT_UPBEAT=2` (menus/onboarding), `FQ_MUSIC_CAT_COUNT=3`. `fq_music_track_t` struct: `path` (LittleFS path string) + `name`. API: `fq_music_table_count(cat)`, `fq_music_table_get(cat, index)`, `fq_music_table_pick(cat, tick_count)` — selects track via `tick_count % count` (no combat PRNG — Constitution Priority 0 isolation preserved). (4) HAL extension — `hal_flash_read_file(path, buf, buf_max, bytes_out)`: additive API in `fq_hal/include/hal_flash.h`. Reads an arbitrary file from the LittleFS partition by path. NULL/init guards match existing `hal_flash_read_save()` pattern. Returns `HAL_FLASH_ERR_NOT_FOUND` on `ENOENT`. Used exclusively by `do_music_load()` in `app_main.c` to stream `.mod` files into the SPIRAM buffer. (5) Application layer — Audio mixing in `main/app_main.c`: `s_music_ctx` is a file-scope static `fq_music_ctx_t` (application layer owner — not `game/`). `s_mod_buf` is a `uint8_t *` pointer to a SPIRAM allocation (`heap_caps_malloc(MAX_MOD_FILE_SIZE, MALLOC_CAP_SPIRAM)`) — sole path where `malloc` is used; it is NOT in the fast path. `do_music_load(state, tick_count)`: maps FSM state to music category, calls `fq_music_table_pick()`, frees any prior SPIRAM buffer, allocates new buffer, calls `hal_flash_read_file()`, calls `fq_music_init()` + `fq_music_play()`. `do_music_tick()`: called every main loop iteration (50ms tick = `MUSIC_TICK_SAMPLES=1102u` samples at 22050 Hz). Reads `s_app->music_enabled` and `s_app->music_vol`; if SFX was emitted this tick (`s_sfx_emitted_this_tick`), applies volume ducking: `vol = (vol * 153u) / 256u` (~60%). Calls `fq_music_render()`, then `hal_audio_write_samples()`. `clamp16(int32_t v)` inline helper: clamps int32 to signed 16-bit range — applied to every mixed sample to prevent int16 overflow/wrap. SPSC ring buffer model preserved — `do_music_tick()` and `do_sfx()` are the sole producers and are both called exclusively from the main loop task. (6) Application context — `fq_app_ctx_t` gains `music_enabled` (uint8_t, default 1) and `music_vol` (uint8_t, default 200) in `main/app_fsm.h`. Both fields reuse previously unused trailing padding bytes — struct size is unchanged: 424 bytes on 64-bit host, 412 bytes on 32-bit target. `_Static_assert` continues to pass. `music_enabled` and `music_vol` are NOT serialized to the save wire format; they re-initialize from defaults on every boot via `fq_app_init()`. (7) Host tests: `test_p22_bounds.c` (17 bound tests covering NULL guards, MAX_MOD_FILE_SIZE overflow, volume clamp edge cases, category sentinel, table-pick modulo correctness) and `test_p22_feature.c` (18 feature tests covering full play/stop/render lifecycle, volume scaling, ducking formula, track selection determinism, and music_enabled mute). Total: 35 new tests. All 129 host tests pass (2 pre-existing failures in `test_p19_home_menu_feature` and `test_p19_interactive_bounds` unchanged). 14/14 visual golden baselines pass. idf.py build: successful, binary size TBD post-hardware integration (SPIRAM .mod buffer not linked into flash binary).
