 ...
 *   F4: fq_text_width("Ember",  font_title) ≤ 180 (fits on one line).
 *   F5: fq_text_width("Tide",   font_title) ≤ 180 (fits on one line).
 *   F6: fq_text_width("EmberTide", font_title) > 180 (splits needed).
 *   F7: fq_text_width("Press Any Button", font_small) ≤ 190 (fits display).
 *
 * Tests F3-F7 exercise the asset layer to confirm the font swap and
 * width values are consistent with the two-line layout decision.
 *
...
int main(void)
{
    ...
    /* -------------------------------------------------------------------
     * F4: "Ember" must fit in ≤180 px at the title font.
     * ------------------------------------------------------------------- */
    int16_t w_ember = fq_text_width(font_title, "Ember");
    TEST_ASSERT_TRUE(w_ember <= 180);
    /* Sanity: non-zero */
    TEST_ASSERT_TRUE(w_ember > 0);

    /* -------------------------------------------------------------------
     * F5: "Tide" must fit in ≤180 px at the title font.
     * ------------------------------------------------------------------- */
    int16_t w_tide = fq_text_width(font_title, "Tide");
    TEST_ASSERT_TRUE(w_tide <= 180);
    TEST_ASSERT_TRUE(w_tide > 0);

    /* -------------------------------------------------------------------
     * F6: "EmberTide" must exceed 180 px — confirms split is needed.
     * ------------------------------------------------------------------- */
    int16_t w_full = fq_text_width(font_title, "EmberTide");
    TEST_ASSERT_TRUE(w_full > 180);

    /* -------------------------------------------------------------------
     * F7: "Press Any Button" must fit on screen at small font.
     * ------------------------------------------------------------------- */
