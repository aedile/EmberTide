 ...
 *
 * Uses TEST_ASSERT_EQUAL_UINT32 / TEST_ASSERT_EQUAL_INT for comparisons.
 */
 ...
 * All glyphs are 8 pixels wide x 8 pixels tall.
 *
 * glyph_index = ch - 32
...
static void test_ascii_boundaries_valid_space_advances_cursor(void)
{
    init_test_font_tables();
    fq_font_t font = make_test_font();
    fq_fb_t fb;
...
static void test_nonprintable_in_middle_skipped(void)
{
    ...
    /* "A\x01\x02B" — two non-printables between 'A' and 'B'.
     * Each printable char has width=8. Cursor advances 8+8 = 16.
     */
    char str[5];
    str[0] = 'A';
    str[1] = 0x01;
...
static void test_cursor_uses_int16_not_uint8(void)
{
    ...
    /* 33 chars x 8 pixels each = 264 pixels. uint8_t would wrap at 256. */
    char long_str[34];
...
static void test_text_stops_at_right_edge(void)
{
    ...
    /* 200-char string of 'A' (width=8 each) starting at x=0.
     * Exactly 25 chars fit (25*8=200). Cursor stops at 200. */
    char str[201];
    memset(str, 'A', 200);
    str[200] = '\0';
...
int main(void)
{
    ...
    test_ascii_boundaries_valid_space_advances_cursor();
