...
int main(void)
{
    ...
    ASSERT_EQ(err, HAL_EPAPER_ERR_NULL, "B1 NULL before init");
    ...
    ASSERT_EQ(err, HAL_EPAPER_ERR_NULL, "B9 NULL check before size check");
    ...
    ASSERT_EQ(err, HAL_EPAPER_ERR_SIZE, "B2 size -1");
    ...
    ASSERT_EQ(err, HAL_EPAPER_ERR_SIZE, "B4 size 0");
    ...
    ASSERT_EQ(err, HAL_EPAPER_ERR_SIZE, "B5 UINT32_MAX size");
    ...
    ASSERT_EQ(err, HAL_EPAPER_ERR_INIT, "B3 not initialised");
    ...
    err = hal_epaper_flush(fb, HAL_EPAPER_FB_SIZE);
    ASSERT_EQ(err, HAL_EPAPER_ERR_SPI, "B6 SPI error injection");

    /* -----------------------------------------------------------------------
     * B8: SPI error is one-shot — subsequent flush succeeds.
     * ----------------------------------------------------------------------- */
    err = hal_epaper_flush(fb, HAL_EPAPER_FB_SIZE);
    ASSERT_EQ(err, HAL_EPAPER_OK, "B8 one-shot SPI error clears");
    ...
    err = hal_epaper_flush(fb, HAL_EPAPER_FB_SIZE);
    ASSERT_EQ(err, HAL_EPAPER_ERR_BUSY_TIMEOUT, "B7 busy timeout injection");
    ...
    ASSERT_EQ(err, HAL_EPAPER_OK, "B10a sleep before init");
