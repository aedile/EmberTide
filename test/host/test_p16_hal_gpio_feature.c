...
int main(void)
{
    ...
    ASSERT_EQ(err, HAL_GPIO_OK, "F1 init OK");
    ...
    ASSERT_EQ(s_fire_count_cb1,                    1u,           "F2 callback fired once");
    ASSERT_EQ((uint32_t)s_last_btn_cb1,            (uint32_t)HAL_BTN_A, "F2 callback btn is A");
    ASSERT_EQ(mock_gpio_get_press_count(HAL_BTN_A), 1u,          "F2 press_count_A == 1");
    ASSERT_EQ(mock_gpio_get_press_count(HAL_BTN_B), 0u,          "F2 press_count_B still 0");

    /* -----------------------------------------------------------------------
     * F3: Simulate press BTN_B — BTN_B counter increments independently.
     * ----------------------------------------------------------------------- */
    mock_gpio_simulate_press(HAL_BTN_B);
    ASSERT_EQ(s_fire_count_cb1,                    2u,           "F3 callback total fires 2");
    ASSERT_EQ((uint32_t)s_last_btn_cb1,            (uint32_t)HAL_BTN_B, "F3 callback btn is B");
    ASSERT_EQ(mock_gpio_get_press_count(HAL_BTN_B), 1u,          "F3 press_count_B == 1");
    ASSERT_EQ(mock_gpio_get_press_count(HAL_BTN_A), 1u,          "F3 press_count_A unchanged");
    ...
    ASSERT_EQ(mock_gpio_get_press_count(HAL_BTN_A), 10u, "F4 burst_10_A count == 10");
    ASSERT_EQ(s_fire_count_cb1,                    10u, "F4 burst_10_A callback fires 10");
    /* BTN_B must be unaffected by BTN_A burst. */
    ASSERT_EQ(mock_gpio_get_press_count(HAL_BTN_B),  0u, "F4 BTN_B unaffected by A burst");
    ...
    ASSERT_EQ(mock_gpio_get_press_count(HAL_BTN_B), 10u, "F5 burst_10_B count == 10");
    ASSERT_EQ(mock_gpio_get_press_count(HAL_BTN_A), 10u, "F5 BTN_A count unchanged at 10");
    ASSERT_EQ(s_fire_count_cb1,                    20u, "F5 total callback fires 20");

    /* -----------------------------------------------------------------------
     * F6: Deinit + reinit cycle restores full operation.
     * ----------------------------------------------------------------------- */
    err = hal_gpio_deinit();
    ASSERT_EQ(err, HAL_GPIO_OK, "F6 deinit OK");

    s_fire_count_cb1 = 0u;
    err = hal_gpio_init(callback1);
    ASSERT_EQ(err, HAL_GPIO_OK, "F6 reinit OK");
    ...
    mock_gpio_simulate_press(HAL_BTN_A);
    ASSERT_EQ(s_fire_count_cb1, 1u, "F6 callback fires after reinit");
    ...
    mock_gpio_simulate_press(HAL_BTN_A);
    ASSERT_EQ(s_fire_count_cb2, 1u,           "F7 cb2 fires after double-init");
    ASSERT_EQ(s_fire_count_cb1, 0u,           "F7 cb1 NOT called after double-init");
    ASSERT_EQ((uint32_t)s_last_btn_cb2, (uint32_t)HAL_BTN_A, "F7 cb2 receives BTN_A");

    mock_gpio_simulate_press(HAL_BTN_B);
    ASSERT_EQ(s_fire_count_cb2, 2u,           "F7 cb2 fires for BTN_B too");
    ASSERT_EQ(s_fire_count_cb1, 0u,           "F7 cb1 still not called");
