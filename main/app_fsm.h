 ...
 *
 * Phase-20 audit fix (DC-1): last_combat_hash added. Populated by
 * fq_generate_combat_hash() on each COMBAT_ROUND_COMPLETE event.
 * Ready for BLE hash exchange protocol.
 *
 * Phase-21: sfx_enabled added.
 *   - 1 = SFX active (default on boot).
 *   - 0 = quiet mode (caller skips fq_sfx_play()).
 *   - Lives here (application layer) because sfx.h is game layer and must
 *     not have quiet-mode awareness (single-responsibility).
 *   - Added as uint8_t after last_combat_hash. On 64-bit host, struct
 *     alignment is 8 bytes (pointer fields), so sizeof rounds 425 → 432.
 *     On 32-bit target, alignment is 4, so 409 → 412.
 * ---------------------------------------------------------------------------*/
...
typedef struct {
     ...
     * Generated by fq_generate_combat_hash() on each COMBAT_ROUND_COMPLETE.
     * Exchanged with peer via FQ_PKT_ROUND_HASH to detect desync. */
    ...
    /* Explicit padding to maintain natural struct alignment.
     * After last_combat_hash (uint32_t at offset 420/404), sfx_enabled adds
     * 1 byte. The compiler pads to the struct's alignment requirement:
     *   64-bit host: alignment=8, 425 → 432 (7 pad bytes).
     *   32-bit target: alignment=4, 409 → 412 (3 pad bytes).
     * Explicit pads below make the layout self-documenting. */
    uint8_t         _p21_pad[3];           /**< Explicit 3-byte pad (32-bit layout). */
} fq_app_ctx_t;
 ...
 * initializes home_menu_index to 0, sfx_enabled to 1, and performs the
 ...
 *
 * Phase-20 audit fix (DC-1):
 *   - last_combat_hash        (uint32_t)       — 4 bytes → 424 (64-bit), 412 (32-bit)
 *
 * Phase-21 additions:
 *   - sfx_enabled             (uint8_t)        — 1 byte at offset 420 (64-bit)
 *   - _p21_pad[3]             (uint8_t[3])     — 3 bytes at offset 421
 *   On 64-bit host the struct ends at 424 (a multiple of 8, so no tail padding).
 *   The Phase-21 fields fit within the 4 bytes of implicit alignment space that
 *   existed after last_combat_hash (which ended at offset 420). The pointer-heavy
 *   layout causes last_combat_hash to land at 416 (not 420 as predicted), so the
 *   struct does not grow beyond 424.
 *
 *   64-bit host verified size: 424 bytes (confirmed by compiler).
 *   32-bit target: last_combat_hash at offset 404; adding 4 bytes → 408 + 4 = 412.
 * ---------------------------------------------------------------------------*/
